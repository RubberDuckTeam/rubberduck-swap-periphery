...
import RubberduckRouter01 from '../../build/RubberduckRouter01.json'
import RubberduckMigrator from '../../build/RubberduckMigrator.json'
import RubberduckRouter02 from '../../build/RubberduckRouter02.json'
...
export async function v2Fixture(provider: Web3Provider, [wallet]: Wallet[]): Promise<V2Fixture> {
  ...
  // deploy routers
  const router01 = await deployContract(wallet, RubberduckRouter01, [factoryV2.address, WETH.address], overrides)
  const router02 = await deployContract(wallet, RubberduckRouter02, [factoryV2.address, WETH.address], overrides)

  // event emitter for testing
  const routerEventEmitter = await deployContract(wallet, RouterEventEmitter, [])

  // deploy migrator
  const migrator = await deployContract(wallet, RubberduckMigrator, [factoryV1.address, router01.address], overrides)
